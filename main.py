...
def make_new_file_mount():

    zalishok = {}
    d1 = pd.read_excel('ostatki.xlsx', engine='openpyxl')
    ...
    d2 = pd.read_excel('NEW_UPDATE_FILE.xlsx',  engine='openpyxl')
    #
    ...
    data.to_excel("NEW_UPDATE_FILE.xlsx", index=False)



def make_new_file_price(path:str):
    ...
    d2 = pd.read_excel('export-products-22-04-22_17-27-35.xlsx',  engine='openpyxl')
    index = 0
...
def main():
    ...
    if len(d3) > 0:
        ...
        data.to_excel('OUT.xlsx', index=False, header=header)
